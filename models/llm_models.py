...
This module provides integrations with various language model providers and frameworks,
enabling agents to use a wide range of models for reasoning, planning, and tool use.

The module includes:
- Base Model class that defines the common interface for all model implementations
- Implementation classes for different providers (OpenAI, HuggingFace, LiteLLM, etc.)
- Utilities for message handling, serialization, and model-specific optimizations
- Classes for working with tool calling capabilities of language models

Each model implementation handles provider-specific details while presenting a unified
interface to the agent framework, allowing seamless switching between different models.
...
def convert_nested_dataclasses_to_dict(obj, ignore_key=None):
    def convert(obj):
...
class ChatMessage:
    ...
    def model_dump_json(self):
        return json.dumps(convert_nested_dataclasses_to_dict(self, ignore_key="raw"))
    ...
    def to_dict(self):
        return json.dumps(convert_nested_dataclasses_to_dict(self))


def parse_json_argument(arguments: Union[str, dict]) -> Union[str, dict]:
...
def get_clean_message_list(
    ...
    for message in message_list:
        role = message["role"]
        if role not in MessageRole.get_all_roles():
            raise ValueError(f"Incorrect role {role}, only {MessageRole.get_all_roles()} are supported for now.")
...
def get_tool_call_from_text(text: str, tool_name_key: str, tool_arguments_key: str) -> ChatMessageToolCall:
    ...
    tool_arguments = parse_json_argument(tool_arguments)
    ...
    def postprocess_message(self, message: ChatMessage, tools_to_call_from) -> ChatMessage:
        ...
        if tools_to_call_from:
            ...
            for tool_call in message.tool_calls:
                tool_call.function.arguments = parse_json_argument(tool_call.function.arguments)
        return message
