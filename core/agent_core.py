...
def extract_template_variables(template_string: str) -> Set[str]:
    pattern = re.compile(r"\{\{([^{}]+)\}\}")
    return {match.group(1).strip() for match in pattern.finditer(template_string)}


def render_template(template_string: str, variables: Dict[str, Any]) -> str:
    compiled_template = Template(template_string, undefined=StrictUndefined)
    try:
        return compiled_template.render(**variables)
    except Exception as e:
        raise Exception(f"Error during template rendering: {type(e).__name__}: {e}")
...
class MultiStepAgent:
    ...
    def __init__(
        ...
        self.logger = AgentLogger(verbosity_level)
        self.agent_memory = AgentMemory()
        self.tool_calls = 0
        self.callbacks = step_callbacks or []
        ...
        self.step_callbacks = self.callbacks + [self.monitor.update_metrics]
