...
This module provides the foundation for creating, managing, and using tools within the
agent framework. Tools are the primary way for agents to interact with external systems,
APIs, and perform specialized functions.

The module includes:
- The base Tool class that all tools inherit from
- Decorators and utilities for creating tools easily
- Helper functions for tool validation and conversion
- Classes for specific tool types (PipelineTool)
- Utilities for loading tools from the Hugging Face Hub

Tools follow a standardized interface with proper input/output typing, documentation,
and validation to ensure consistent behavior within the agent ecosystem.
...
class Tool:
    ...
    def validate_arguments(self):
        ...
        for input_name, input_config in self.inputs.items():
            assert isinstance(input_config, dict), f"Input '{input_name}' should be a dictionary."
            assert "type" in input_config and "description" in input_config, (
                f"Input '{input_name}' should have keys 'type' and 'description', has only {list(input_config.keys())}."
            )
            if input_config["type"] not in VALID_TYPES:
                raise Exception(
                    f"Input '{input_name}': type '{input_config['type']}' is not valid, should be one of {VALID_TYPES}."
                )

        assert getattr(self, "output_type", None) in VALID_TYPES
        ...
        if not (
            ...
            if not set(key for key in signature.parameters.keys() if key != "self") == set(self.inputs.keys()):
                raise TypeError(
                    f"The forward function should have the same arguments as the inputs. Got {signature.parameters.keys()} and {self.inputs.keys()}."
                )
    ...
    def from_gradio(gradio_tool):
        ...
        class GradioToolWrapper(Tool):
            def __init__(self, _gradio_tool):
                ...
                self.inputs = {
                    key: {"type": TYPE_MAPPING[value.annotation], "description": ""} for key, value in func_args
                }
